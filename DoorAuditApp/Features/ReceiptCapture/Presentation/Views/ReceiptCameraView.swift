...
enum ScannerGuidanceState: Equatable {
    ...
    var message: String {
        ...
        case .tooDark:
            return "More light needed before the scan can auto-capture."
        case .tooBright:
            return "Reduce glare or bright reflections on the receipt."
        case .blurry:
            return "Hold steady until the receipt edges sharpen."
...
class CameraViewController: UIViewController {
    ...
    private let imageQualityService = CoreImageQualityService()
    ...
    private func handleLiveDetectionResult(_ document: DetectedDocument?, pixelBuffer: CVPixelBuffer) {
        ...
        if let qualityFeedback = imageQualityService.analyze(pixelBuffer: pixelBuffer) {
            if qualityFeedback.issues.contains(.tooDark) {
                stableFrameCount = 0
                publishGuidance(.tooDark)
                return
            }

            if qualityFeedback.issues.contains(.tooBright) {
                stableFrameCount = 0
                publishGuidance(.tooBright)
                return
            }

            if qualityFeedback.issues.contains(.blurry) {
                stableFrameCount = 0
                publishGuidance(.blurry)
                return
            }
        }
...
extension CameraViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
        ...
        handleLiveDetectionResult(document, pixelBuffer: pixelBuffer)
    }
}
